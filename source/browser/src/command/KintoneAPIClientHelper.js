...
class KintoneAPIClientHelper {
    ...
    static async findBy({app = kintone.app.getId(), query = '', tokens}) {
        return KintoneAPIClientHelper.of(app, tokens).__get({query, totalCount: true});
    }
    /**
     * 条件なし 全件 検索
     * @param {object} param
    ...
    * @returns {Promise<{ids: [], revisions:[]}>} 更新結果
    ...
    async __get(params = {}) {
        ...
        /* 実行内容別 に処理実行 */
        if (Object.hasOwn(_params, 'id')) return this.__getById(_params, this.tokens); // ID指定
        else if (query.includes('limit')) return this.__getAll(_params, this.tokens); // 上限指定
                ...
                kintone.api(
                    ...
                    async (error) => {
                        if (tokens) {
                            ...
                            const baseURI = kintone.api.url('/k/v1/record.json', true).replaceAll('.s.', '.'); // Base URI = クライアント証明書でないcybozu URL
                            ...
                            resolve(response && response.length ? JSON.parse(response[0]) : {record: {}});
                        } else {
                            reject(error);
                        }
                    }
                ...
                kintone.api(
                    ...
                    async (error) => {
                        if (tokens) {
                            // kintone.proxyにて通信
                            const _tokens = Array.isArray(tokens) ? tokens : [tokens]; // 補完
                            const baseURI = kintone.api.url('/k/v1/records.json', true).replaceAll('.s.', '.'); // Base URI = クライアント証明書でないcybozu URL
                            const headers = { 'X-Cybozu-API-Token': _tokens.join(',') }; // リクエストヘッダー = APIトークン指定
                            const url = `${baseURI}?${new URLSearchParams(params)}`; // 通信API

                            // 通信実行
                            const response = await kintone.proxy(url, 'GET', headers, {});
                            resolve(response && response.length ? JSON.parse(response[0]) : {records: [], totalCount: null});
                        } else {
                            reject(error);
                        }
                    }
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                while (true) {
                    ...
                    const _params = {
                        ...params,
                        query: `${query} ${query.includes('offset') ? '' : `offset ${offset}`}`, // 全件取得用
                    };
    ...
    async __post(record, tokens) {
        try {
            // overlaod
            const { ids = [], revisions = []} = await this.__postAll([record], tokens);
            // 結果1件
                    ...
                    kintone.api(
                        ...
                        async (error) => {
                            if (tokens) {
                                // kintone.proxyにて通信
                                const _tokens = Array.isArray(tokens) ? tokens : [tokens]; // 補完
                                const headers = { 'X-Cybozu-API-Token': _tokens.join(','), 'Content-Type': 'application/json' }; // リクエストヘッダー
                                const url = kintone.api.url('/k/v1/records.json', true).replaceAll('.s.', '.'); // 通信API = クライアント証明書でないcybozu URL

                                // 通信実行
                                const response = await kintone.proxy(url, 'POST', headers, body);
                                resolve(response && response.length ? JSON.parse(response[0]) : {ids: [], revisions: []});
                            } else {
                                reject(error);
                            }
                        }
                    ...
                    kintone.api(
                        ...
                        async (error) => {
                            if (tokens) {
                                ...
                                resolve(response && response.length ? JSON.parse(response[0]) : []);
                            } else {
                                reject(error);
                            }
                        }
                    ...
                    kintone.api(
                        ...
                        async (error) => {
                            if (tokens) {
                                ...
                                // 通信実行
                                const response = await kintone.proxy(url, 'DELETE', headers, body);
